...
import { accent, alpha, fill, heat, ink, success } from '../styles/tokens';
    ...
    if (mode === 'heat_cool') {
      return (
        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <span style={{ color: heat }}>{targetLow?.toFixed(0) ?? '--'}°</span>
          <span style={{ color: ink.ghost }}>-</span>
          <span style={{ color: accent }}>{targetHigh?.toFixed(0) ?? '--'}°</span>
        </span>
      );
    }
    ...
    >
      ...
      <div style={styles.climateHeaderStyle}>
        <div
          style={{
            ...
            backgroundColor: isHeating ? alpha(heat, 0.2) : isCooling ? alpha(accent, 0.2) : fill.hairline,
      ...
      <div style={styles.climateControlRowStyle}>
        {['off', 'heat', 'cool', 'heat_cool'].map(m => {
          ...
          const colors: Record<string, string> = { off: ink.primary, heat: heat, cool: accent, heat_cool: success };
