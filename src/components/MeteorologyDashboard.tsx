      ...
      <div style={styles.mainGridStyle}>
        ...
        <div style={styles.scrollableColumnStyle}>
          ...
          <GlassCard title="Atmosphere">
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:thermometer" label="Temp" value={`${temp.state}°C`} color="#ff9800" />
              <BigMetric icon="mdi:account-question" label="Feels Like" value={`${feelsLike.state}°C`} color="#ff5722" />
              <BigMetric icon="mdi:water-percent" label="Humidity" value={`${humidity.state}%`} color="#03a9f4" />
              <BigMetric icon="mdi:thermometer-water" label="Dew Point" value={`${dewPoint.state}°C`} color="#00bcd4" />
            </div>
          </GlassCard>
          ...
          <GlassCard title={skyCondition.state?.replace('_', ' ') || "Sky Status"}>
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:clouds" label="Sky Avg" value={`${skyAvgTemp.state}°C`} color="#9c27b0" />
              <BigMetric icon="mdi:target" label="Sky Center" value={`${skyCenterTemp.state}°C`} color="#673ab7" />
              <BigMetric icon="mdi:brightness-6" label="Brightness" value={skyBrightness.state} color="#ffeb3b" />
              <BigMetric icon="mdi:thermometer-chevron-up" label="Sky Max" value={`${skyMaxTemp.state}°C`} color="#f44336" />
              <BigMetric icon="mdi:thermometer-chevron-down" label="Sky Min" value={`${skyMinTemp.state}°C`} color="#3f51b5" />
            </div>
          </GlassCard>
        </div>
        ...
        <div style={styles.scrollableColumnStyle}>
          ...
          <GlassCard title="Wind Speed & Direction">
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:weather-windy" label="Speed" value={`${windSpeed.state} km/h`} color="#4caf50" />
              <BigMetric icon="mdi:wind-power" label="Gust" value={`${windGust.state} km/h`} color="#8bc34a" />
              <BigMetric icon="mdi:compass-outline" label="Direction" value={`${windDir.state}°`} color="#cddc39" />
              <BigMetric icon="mdi:speedometer" label="Max Gust" value={`${maxGust.state} km/h`} color="#4caf50" />
              <BigMetric icon="mdi:weather-windy-variant" label="Sky Wind" value={`${skyAnemometerWind.state} km/h`} color="#81c784" />
            </div>
          </GlassCard>
          ...
          <GlassCard title="Pressure Readings">
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:gauge" label="Relative" value={`${relPressure.state} hPa`} color="#607d8b" />
              <BigMetric icon="mdi:gauge-empty" label="Absolute" value={`${absPressure.state} hPa`} color="#455a64" />
            </div>
          </GlassCard>
          ...
          <GlassCard title="Radiation">
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:sun-wireless" label="UV Index" value={uvIndex.state} color="#ffeb3b" />
              <BigMetric icon="mdi:brightness-5" label="Illuminance" value={`${Number(illuminance.state).toLocaleString()} lx`} color="#ffc107" />
              <BigMetric icon="mdi:solar-power" label="Irradiance" value={`${irradiance.state} W/m²`} color="#ff9800" />
            </div>
          </GlassCard>
        </div>
        ...
        <div style={styles.scrollableColumnStyle}>
          ...
          <GlassCard title={`Rain Status: ${skyRainSensor.state}`}>
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:weather-rainy" label="Daily" value={`${dailyRain.state} mm`} color="#2196f3" />
              <BigMetric icon="mdi:weather-pouring" label="Intensity" value={`${precipIntensity.state} mm/h`} color="#00bcd4" />
              <BigMetric icon="mdi:numeric" label="Sky Rain" value={skyRainNumeric.state} color="#03a9f4" />
              <BigMetric icon="mdi:calendar-week" label="Weekly" value={`${weeklyRain.state} mm`} color="#03a9f4" />
              <BigMetric icon="mdi:calendar-star" label="Event" value={`${eventRain.state} mm`} color="#4caf50" />
              <BigMetric icon="mdi:calendar-month" label="Monthly" value={`${monthlyRain.state} mm`} color="#3f51b5" />
              <BigMetric icon="mdi:calendar-range" label="Yearly" value={`${yearlyRain.state} mm`} color="#673ab7" />
              <BigMetric icon="mdi:history" label="Last Rain" value={lastRain.state} color="#9e9e9e" />
            </div>
          </GlassCard>
          ...
          <GlassCard title="Indoor & Sky Sensors">
            <div style={styles.cardGridStyle}>
              <BigMetric icon="mdi:home-thermometer" label="Sky Amb T" value={`${skyAmbientTemp.state}°C`} color="#ff5722" />
              <BigMetric icon="mdi:home-water" label="Sky Amb H" value={`${skyAmbientHum.state}%`} color="#03a9f4" />
            </div>
          </GlassCard>
        </div>
      </div>
